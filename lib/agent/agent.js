        ...
        async.waterfall([
            ...
            function renderFiles(manifests, metadata, next) {
                ...
                Object.keys(manifests).forEach(function (name) {
                    self.renderConfigFile(log.child({
                        manifest: name
                    }, true),
                    manifests[name],
                    metadata,
            ...
            function writeFiles(manifests, next) {
                ...
                vasync.forEachParallel({
                    func: function (name, nextManifest) {
                        self.writeConfigFile(log.child({
                            manifest: name
                        }, true),
                        manifests[name],
                        nextManifest);
...
Agent.prototype.renderConfigFile =
    function renderConfigFile(
        log, manifest, rawMetadata, autoInstanceMetadata) {
    var self = this;
    ...
    async.waterfall([
        ...
        function (updated, subcb) {
            ...
            if (os.platform() === 'linux') {
                if (!manifest.post_cmd_linux) {
                    log.trace('no post_cmd_linux');
                    subcb(null);
                    return;
                }
                self.runPostCommand(log, manifest.post_cmd_linux,
                function (_err) {
                    /*
