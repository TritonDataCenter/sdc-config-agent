...
function Agent(config, log) {
	 ...
	 *
	 * Since config-agent has been updated to work on Compute Nodes, and thus
	 * being able to handle multiple instances, localManifestDirs can have an
	 * additional form where the instance UUIDs are known and don't correspond
	 * to `zonename`. The regular (since instance) format for localManifestDirs
	 * goes like this:
	 *
	 * localManifestDirs: [ "/path/one", "/path/two", "/path/three", .. ]
	 *
	 * For supporting more than one instance, localManifestDirs can also adopt
	...
	/*
	 * agent.fullCheckRefresh allows us to force checkAndRefresh to run
	 * completely so agent.renderFiles, agent.writeFiles [and
	 * agent.runPostCommand] will be executed regardless of the state of the
	 * config objects returned by SAPI. This is only useful when running the
	 * agent instance within an interval loop where we want to checkAndRefresh
	 * repeatedly. By using fullCheckRefresh=false the agent will check the
	 * etag of the returned config objects and avoid continuing with the rest of
	 * checkAndRefresh if the etags haven't changed.
	 *
	 * Use the default 'true' value for fullCheckRefresh if you want to keep the old
	 * config-agent behavior
	 */
	this.fullCheckRefresh = config.fullCheckRefresh || true;
...
Agent.prototype.init = function init(cb) {
	...
	// vm instance behaviour: if localManifestDirs is an array, transform the
	// variable to the general { uuid: localManifestDirs } object. vm instances
	// can only have one instance configuratio so we use instances[0]
	if (Array.isArray(this.localManifestDirs)) {
		assert.arrayOfString(this.localManifestDirs);

		var localManifestDirs = {};
		localManifestDirs[self.instances[0]] = this.localManifestDirs;
		this.localManifestDirs = localManifestDirs;
	}

	vasync.forEachParallel({
		func: function (instance, subsubcb) {
			self.initInstance.call(self, instance, subsubcb);
		},
...
Agent.prototype.initInstance = function initInstance(instance, cb) {
	...
	findManifestDirs.call(this, this.localManifestDirs[instance],
	function (err, dirs) {
		if (err)
			return (cb(err));
		...
		vasync.forEachParallel({
			...
			log.debug({ localManifests: manifests }, 'local manifests');
...
function checkAndRefreshInstance(instance, cb) {
	...
	sapi.getConfig(instance, opts, function (err, config, req, res) {
		...
		if (res.statusCode === 304) {
			log.debug('configuration for instance %s has not changed', instance);
			return cb();
		}
		...
		async.waterfall([
			function resolveLocalManifests(subcb) {
				var manifests = resolveManifests.call(self,
					config.manifests, self.localManifests[instance] || []);

				return (subcb(null, manifests, config.metadata));
			},
			function renderFiles(manifests, metadata, subcb) {
				...
				Object.keys(manifests).forEach(function (name) {
					renderConfigFile.call(self, manifests[name], metadata);
				});
			...
			function writeFiles(manifests, subcb) {
				...
				vasync.forEachParallel({
					func: function (name, subsubcb) {
						writeConfigFile.call(self, manifests[name], subsubcb);
					},
			...
			if (err) {
				log.error(err,
					    'failed to write files for instance %s',
					    instance);
			}
			cb(err);
		});
...
function resolveManifests(sapiManifests, localManifests) {
	...
	log.debug({ manifests: manifests }, 'resolved SAPI and local manifests');
...
function renderConfigFile(manifest, metadata) {
	...
	log.debug('rendering configuration file from manifest %s', manifest.name);
	...
	if (!contents) {
		...
		log.debug({ contents: contents }, 'rendered configuration file');
	}
	...
	async.waterfall([
		function (subcb) {
			...
			mkdirp(dirname, function (err) {
				if (err) {
					log.warn(err, 'failed to mkdir -p %s', dirname);
				}
		...
		function (subcb) {
			fs.readFile(manifest.path, 'ascii', function (err, file) {
				if (err) {
					log.warn(err, 'failed to read file %s', manifest.path);
					return (subcb(null));
				}
		...
		function (updated, subcb) {
			...
			runPostCommand.call(self, manifest.post_cmd, function (err) {
				/*
